...
std::string resolve_path(std::string_view path, std::string_view baseFilePath)
{
    ...
    if (is_absolute_path(path))
    {
        ...
        pathComponents = split_path(path.substr(afterPrefix));
    }
    else
    {
        pathComponents = split_path(path);
        if (is_absolute_path(baseFilePath))
        {
            ...
            baseFilePathComponents = split_path(baseFilePath.substr(afterPrefix));
        }
        else
        {
            // path and baseFilePath are both relative, we do not set a prefix (resolved path will be relative)
            baseFilePathComponents = split_path(baseFilePath);
        }
    }
